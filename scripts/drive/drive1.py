    ...
    def image_callback(self, msg):
        ...
        if M2['m00'] > 0 :   #fisrt detect
            if self.stopable == 0:                          # 0: True, 1:False
                if self.linecount==2 or self.linecount==3 or self.linecount==5 or self.linecount==9:
...
#---stopline
        mask_white[0:h-40, 0:w] = 0
        mask_white[h-20:h, 0:w] = 0
        mask_white[h-40:h-20,20:w]=0
        ...
        if M['m00'] > 0:
            ...
            if M_w['m00'] > 0 and self.stoplv==0:
                cv2.circle(right_yellow_image, (cx_white, cy_white), 20, (0, 255, 255), -1)
                self.stoplv == 1
                self.twist.linear.x = 0.0
                rospy.sleep(3.0)
